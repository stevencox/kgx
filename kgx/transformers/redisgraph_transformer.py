...
class RedisgraphTransformer(Transformer):
    """
    Transformer for reading from and writing to a Redisgraph database.
    ...
    source_graph: Optional[networkx.MultiDiGraph]
        The source graph
    uri: Optional[str]
        The Redisgraph URI (with port)
    username: Optional[str]
        The Redisgraph username for authentication
    password: Optional[str]
        The Redisgraph password for authentication
    ...
    def load(self, start: int = 0, end: Optional[int] = None, is_directed: bool = True, page_size: int = 50000, provided_by: Optional[str] = None) -> None:
        """
        Read nodes and edges from a Redisgraph database and create a networkx.MultiDiGraph
        ...
        page_size: int
            Size of page (or chunk) to fetch from Redisgraoh
        provided_by: Optional[str]
            Define the source providing the data
        """
        if end is None:
            # get total number of records to be fetched from Redisgraph
        ...
        with click.progressbar(length=count, label='Getting {:,} records from Redisgraph'.format(count)) as bar:
    ...
    def count(self, is_directed: bool = True) -> int:
        """
        Get the total count of records to be fetched from the Redisgraph database.
        ...
        query_result: Any
        try:
            query_result = self.redisgraph.query(query)
        except ResponseError as ce:
            log.error(ce)
    ...
    def get_pages(self, query_function, start: int = 0, end: Optional[int] = None, page_size: int = 50000, **kwargs: Dict) -> Iterator:
        """
        Get pages of size ``page_size`` from Redisgraph.
        ...
        -------
        Iterator
            An iterator for a list of records from Redisgraph. The size of the list is ``page_size``
    ...
    def get_nodes(self, skip: int = 0, limit: int = 0) -> List:
        """
        Get a page of nodes from the Redisgraph database.
        ...
        log.debug(query)
        try:
            results = self.redisgraph.query(query)
        except ResponseError as ce:
            log.error(ce)
        if results:
    ...
    def get_edges(self, skip: int = 0, limit: int = 0, is_directed: bool = True) -> List:
        """
        Get a page of edges from the Redisgraph database.
        ...
        try:
            ...
            log.debug(f"Time taken to fetch edges from Redisgraph: {end - start} ms")
        except ResponseError as ce:
            log.error(ce)
        if results:
    ...
    def save_node(self, nodes_by_category: Dict[str, list], batch_size: int = 10000) -> None:
        """
        Save all nodes into Redisgraph using the UNWIND cypher clause.
            ...
            for x in range(0, len(nodes), batch_size):
                ...
                for node in batch:
                    try:
                        ...
                        txt = ",".join (txt)
                        props = f"{{ {txt} }}"
                        cat = category.split ("|")[0].replace ("`", "")
                        command = f"CREATE (:{cat} {props})"
                        self.redisgraph.query (command)
                    except ResponseError as ce:
                        log.error(ce)
                self.redisgraph.commit ()
            time_end = current_time_in_millis()
    ...
    def generate_unwind_node_query(category: str) -> str:
        """
        Generate UNWIND cypher query for saving nodes into Redisgraph.

        There should be a CONSTRAINT in Redisgraph for ``self.DEFAULT_NODE_CATEGORY``.
    ...
    def save_edge(self, edges_by_edge_label: Dict[str, list], batch_size: int = 10000) -> None:
        """
        Save all edges into Redisgraph using the UNWIND cypher clause.
                ...
                for edge in batch:
                    try:
                        ...
                        command = "\n".join ([
                            f" MATCH (subject) ",
                            f" MATCH (object) ",
                            f" WHERE subject.id = '{subject}' ",
                            f"   AND object.id  = '{object}' ",
                            f"CREATE (subject)-[:{predicate} {props}]->(object)" ])
                        log.debug (command)
                        self.redisgraph.query (command)
                    except ResponseError as ce:
                        log.error(ce)
    ...
    def generate_unwind_edge_query(edge_label: str) -> str:
        """
        Generate UNWIND cypher query for saving edges into Redisgraph.
        ...
        MATCH (s:`{RedisgraphTransformer.DEFAULT_NODE_CATEGORY}` {{id: edge.subject}}), (o:`{Transformer.DEFAULT_NODE_CATEGORY}` {{id: edge.object}})
    ...
    def save(self) -> None:
        """
        Save all nodes and edges from networkx.MultiDiGraph into Redisgraph using the UNWIND cypher clause.

        """
        nodes_by_category = {}
        log.debug (f"nodes: {self.graph.nodes(data=True)}")
        ...
        # create indexes
        # TODO: ************************* redisgraph does not support constraints yet.
    ...
    def redisgraph_report(self) -> None:
        """
        Give a summary on the number of nodes and edges in the Redisgraph database.
        ...
        try:
            edge_results = self.redisgraph.query("MATCH (s)-->(o) RETURN COUNT(*)")
    ...
    def create_constraints(self, categories: Union[set, list]) -> None:
        """
        Create a unique constraint on node 'id' for all ``categories`` in Redisgraph.
        ...
        for category in categories_set:
            if self.CATEGORY_DELIMITER in category:
                ...
                query = RedisgraphTransformer.create_constraint_query(category)
                try:
                    edge_results = self.redisgraph.query(query)
                except ResponseError as ce:
                    log.error(ce)
